	...
	<div>
		<div>
			<div style="border: 1px solid #cccccc">
				...
				<!-- 地图 -->
				<el-amap class="amap-box" :zoom="amap.zoom" :center="amap.center" :events="amap.events">
					<!-- 当前位置图标 -->
...
<script>
	...
	export default {
		...
		watch: {
			initPoint(val) {
				if (val) {
					this.amap.center = [val.lon, val.lat]
					const obj = {
						position: [val.lon, val.lat],
						text: this.address,
						offset: [0, 30],
					};
					this.searchOption.city = val.city;
					this.dataForm.province = val.province
					this.dataForm.city = val.city
					this.dataForm.district = val.district
					this.amap.markers = [];
					...
					this.amap.center = [this.dataForm.lon, this.dataForm.lat]
					const obj = {
						position: [this.dataForm.lon, this.dataForm.lat],
							...
							geocoder.getAddress([lng, lat], function(status, result) {
								if (status === "complete" && result.info === "OK") {
									if (result && result.regeocode) {
										...
										if (vm.dataForm.province == '北京市' || vm.dataForm.province == '天津市' || vm
											.dataForm.province == '上海市') {
											vm.dataForm.city = vm.dataForm.province.substr(0, 2) + '城区';
										}
										if (result.regeocode.addressComponent.adcode.startsWith('5001')) {//重庆城区
											vm.dataForm.city = vm.dataForm.province.substr(0, 2) + '城区';
										}
										if (result.regeocode.addressComponent.adcode.startsWith('5002')) {//重庆郊县
											vm.dataForm.city = vm.dataForm.province.substr(0, 2) + '郊县';
										}
										...
										vm.searchOption.city = vm.dataForm.city
										vm.sure();
									}
								}
							});
