export const mapOption = {
  basicOption: {
    ...
    tooltip: {//提示框组件
      ...
      // formatter: '{b}',
      formatter: (param) => {
        let val = typeof (param.value) === 'number' ? param.data.value : param.data.value[2];
        if (isNaN(val)) {
          val = 0;
        }
        let val1 = typeof (param.value) === 'number' ? param.data.value2 : param.data.value[3];
        if (isNaN(val1)) {
          val1 = 0;
        }
        
        // localStorage.setItem('iii',JSON.stringify(param))
        return '<div style="text-align:left;padding: 15px;font-weight: bolder;border-radius: 5px;background: linear-gradient(to right, rgba(36, 118, 183, 1) , rgba(36, 118, 183, .5))">' +
          ...
          '</div>' +
          '<div style="padding: 2px 16px;display:flex;align-items: flex-start;flex-direction:column;">' +
          '<div>' + '电站数：' +val+ '</div>' +
          '<div>' + '电桩数：' +val1+ '</div>' +
          '<div style="display:none">'+localStorage.setItem('iii',JSON.stringify(param))+'</div>'+
          '</div>' +
          '</div>';
      }
    },
